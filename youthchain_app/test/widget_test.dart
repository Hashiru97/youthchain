...
void main() {
  ...
  testWidgets('RegistrationScreen renders its title and contact step', (
    ...
    expect(
      find.text('How should we send your verification code?'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'full happy path: contact -> code -> details -> account created',
    (WidgetTester tester) async {
      ...
      final otpRequest = requests.firstWhere(
        (r) => r.url.path == '/auth/otp/register/request',
      );
      ...
      // Step 2: code.
      expect(
        find.textContaining('A 6-digit code was sent to newyouth@test.com'),
        findsOneWidget,
      );
      ...
      final verifyRequest = requests.firstWhere(
        (r) => r.url.path == '/auth/otp/register/verify',
      );
      ...
      // Step 3: details.
      expect(
        find.textContaining('newyouth@test.com is verified'),
        findsOneWidget,
      );
      ...
      await tester.enterText(
        find.widgetWithText(TextField, 'NCRA ID (optional)'),
        'SL-000998877',
      );
      await tester.enterText(
      ...
      final registerRequest = requests.firstWhere(
        (r) => r.url.path == '/register',
      );
