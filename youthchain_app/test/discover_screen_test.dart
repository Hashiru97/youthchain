...
void main() {
  ...
  testWidgets('renders every job returned by the API', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(theme: AppTheme.light(), home: const DiscoverScreen(userId: 1)),
    );
    ...
    expect(find.text('Scraped listing'), findsOneWidget); // job 2's source fallback
  });

  testWidgets('shows the scanner-hasn\'t-run-yet empty state when no search is active', (tester) async {
    ...
    expect(find.text('Check back soon — sources are scanned regularly.'), findsOneWidget);
  ...
  testWidgets('shows a different empty-state subtitle when a search returns nothing', (tester) async {
    ...
    await tester.pump();
    await tester.pump();
    expect(find.text('Junior Developer'), findsOneWidget); // sanity: real jobs loaded first

    await tester.enterText(find.byType(TextField), 'zzz_no_such_job');
    await tester.pump(const Duration(milliseconds: 500)); // clear the 400ms debounce
    await tester.pump();
    await tester.pump();

    expect(find.text('No listings found'), findsOneWidget);
    expect(find.text('Try a different search term.'), findsOneWidget);
    expect(find.text('Check back soon — sources are scanned regularly.'), findsNothing);
  });

  testWidgets('tapping a job card opens DiscoverJobDetailScreen', (tester) async {
    ApiClient.testClient = MockClient((request) async {
      return http.Response(jsonEncode(sampleJobs), 200);
    });
  ...
  testWidgets('search field debounces and requests with a q param', (tester) async {
